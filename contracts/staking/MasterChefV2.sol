...
contract MasterChefV2 is NeedInitialize, OwnableUpgradeable {
  using SafeMath for uint256;
  ...
  mapping(address => uint256) public userBaseClaimableAmountV1;
  ...
  function setmintedTokens(uint256 _mintedTokens) public onlyOwner {
    mintedTokens = _mintedTokens;
  }

  function setuserBaseClaimableAmountV1(address[] memory _users, uint256[] memory _amount) public onlyOwner {
    for (uint256 i; i < _users.length; i++) {
      userBaseClaimableAmountV1[_users[i]] = _amount[i];
    }
  }
  ...
  function claim(address _user, address[] calldata _tokens) external {
    ...
    if(userBaseClaimableAmountV1[_user] != 0){
      pending = pending.add(userBaseClaimableAmountV1[_user]);
      userBaseClaimableAmountV1[_user] = 0;
    }
    uint256 _totalAllocPoint = totalAllocPoint;
    for (uint256 i = 0; i < _tokens.length; i++) {
      PoolInfo storage pool = poolInfo[_tokens[i]];
