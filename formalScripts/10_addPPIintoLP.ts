...
async function main() {
  ...
  // add gPPI as reward
  let tx = await multiFeeDistributionV2.addReward(addresses.Markets.PPI.atoken);
  await tx.wait();
  console.log(`>> ✅ multiFeeDistributionV2.addReward ${tx.hash}`);
  tx = await lendingPoolConfigurator.enableBorrowingOnReserve(addresses.Markets.PPI.token, true);
  console.log(`>> ✅ lendingPoolConfigurator.enableBorrowingOnReserve ${tx.hash}`);
  tx = await lendingPoolConfigurator.configureReserveAsCollateral(addresses.Markets.PPI.token, 6500, 7500, 11000);
  console.log(`>> ✅ lendingPoolConfigurator.configureReserveAsCollateral ${tx.hash}`);
  tx = await lendingPoolConfigurator.setReserveFactor(addresses.Markets.PPI.token, 5000);
