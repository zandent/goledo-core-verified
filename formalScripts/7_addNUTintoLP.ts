...
const SWAPPIPAIR = "0xd9d5748cb36a81fe58f91844f4a0412502fd3105";
...
async function main() {
  ...
  for (const token of ["NUT"]) {
    ...
    if (market.oracle === "") {
      const WitnetPriceFeed = await ethers.getContractFactory("SwappiPairPriceFeed", deployer);
  ...
  // add gNUT as reward
  await multiFeeDistribution.addReward(addresses.Markets.NUT.atoken);
  await lendingPoolConfigurator.enableBorrowingOnReserve(addresses.Markets.NUT.token, true);
  await lendingPoolConfigurator.configureReserveAsCollateral(addresses.Markets.NUT.token, 4000, 5000, 10900);
  await lendingPoolConfigurator.setReserveFactor(addresses.Markets.NUT.token, 5000);
}
